...
const DocumentCard = ({ document, onDelete }) => {
  ...
  // Fix ID from backend (MySQL uses id, not _id)
    ...
    >
      ...
      <div>
        ...
        <div className="flex items-center gap-3">
          {document.flashcard_count !== undefined && (
            ...
            <span className="text-xs font-semibold text-purple-700">{document.flashcard_count} Flashcards</span>
          </div>
          )}
          
          {document.quiz_count !== undefined && (
            <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-purple-50 rounded-lg">
            <BrainCircuit className="w-3.5 h-3.5 text-emerald-600" strokeWidth={2} />
            <span className="text-xs font-semibold text-emerald-700">{document.quiz_count} Quizzes</span>
          </div>
          )}
      ...
      <div className="mt-5 pt-4 border-t border-slate-100">
        <div className="flex items-center gap-1.5 text-slate-500 text-xs">
          ...
          <span>Uploaded {moment(document.upload_date).fromNow()}</span>
        </div>
      </div>

      {/* Hover outline */}
      <div className="absolute inset-0 rounded-2xl bg-linear-to-br from-emerald-500/0 to-teal-500/0 group-hover:from-emerald-500/5 group-hover:to-teal-500/5 transition-all duration-200 pointer-events-none" />
    </div>
  );
};
