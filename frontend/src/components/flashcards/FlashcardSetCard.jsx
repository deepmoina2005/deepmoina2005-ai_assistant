...
const FlashcardSetCard = ({ flashcardSet }) => {
  ...
  const handleStudyNow = () => {
    const documentId =
      flashcardSet?.documentId?._id || flashcardSet?.document_id;
    if (documentId) {
      navigate(`/documents/${documentId}/flashcards`);
    }
  };

  // Safe fallback for cards
  const cards = flashcardSet.cards ?? [];
  const totalCards = cards.length;
  const reviewCount = cards.filter((c) => c.lastReviewed).length;
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h3
              ...
              title={
                flashcardSet?.documentId?.title || flashcardSet?.document_title
              }
            >
              {flashcardSet?.documentId?.title || flashcardSet?.document_title}
            </h3>
            {flashcardSet.created_at && (
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">
                Created {moment(flashcardSet.created_at).fromNow()}
              </p>
            )}
          </div>
        </div>
